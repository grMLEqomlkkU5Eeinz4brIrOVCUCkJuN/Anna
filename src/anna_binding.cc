...
class Anna : public Napi::ObjectWrap<Anna> {
  public:
    static Napi::Object Init(Napi::Env env, Napi::Object exports) {
        Napi::Function func = DefineClass(
            env, "Anna",
            {
                ...
                InstanceMethod("findValidPermutations", &Anna::FindValidPermutations),
    ...
    explicit Anna(const Napi::CallbackInfo &info)
        ...
        if (info.Length() >= 1 && info[0].IsNumber()) {
            size_t threads = static_cast<size_t>(info[0].As<Napi::Number>().DoubleValue());
  ...
  private:
    ...
    Napi::Value Size(const Napi::CallbackInfo &info) {
        return Napi::Number::New(info.Env(), static_cast<double>(finder_.size()));
    }
    ...
    Napi::Value FindAnagrams(const Napi::CallbackInfo &info) {
        Napi::Env env = info.Env();
        if (info.Length() < 1 || !info[0].IsString()) {
            Napi::TypeError::New(env, "Expected input string").ThrowAsJavaScriptException();
            return env.Undefined();
        }
        ...
        for (size_t i = 0; i < result.size(); ++i) {
            out[i] = Napi::String::New(env, result[i].c_str(), result[i].size());
        }
        return out;
    }
    ...
    Napi::Value FindValidPermutations(const Napi::CallbackInfo &info) {
        ...
        if (info.Length() >= 2 && info[1].IsNumber()) {
            ...
            if (d > 0) maxResults = static_cast<size_t>(d);
        }
    ...
    Napi::Value GetWordsByLength(const Napi::CallbackInfo &info) {
        ...
        size_t length = static_cast<size_t>(info[0].As<Napi::Number>().DoubleValue());
    ...
    Napi::Value SetThreadCount(const Napi::CallbackInfo &info) {
        Napi::Env env = info.Env();
        if (info.Length() < 1 || !info[0].IsNumber()) {
            Napi::TypeError::New(env, "Expected threads number").ThrowAsJavaScriptException();
            return env.Undefined();
        }
        size_t threads = static_cast<size_t>(info[0].As<Napi::Number>().DoubleValue());
        finder_.setThreadCount(threads);
    ...
    Napi::Value GetThreadCount(const Napi::CallbackInfo &info) {
        return Napi::Number::New(info.Env(), static_cast<double>(finder_.getThreadCount()));
    }
};

Napi::FunctionReference Anna::constructor;

// Export hook invoked from Seshat.cc
